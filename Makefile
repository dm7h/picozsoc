...
RISCV_TOOLS_PREFIX ?= /opt/riscv32ic/bin/
...
hx8kdemo.asc: hx8kdemo.pcf hx8kdemo.blif
	arachne-pnr -d 8k -P tq144:4k -o hx8kdemo.asc -p hx8kdemo.pcf hx8kdemo.blif

hx8kdemo.bin: hx8kdemo.asc
	icetime -d hx8k -P tq144:4k -c 16 -mtr hx8kdemo.rpt hx8kdemo.asc
	icepack hx8kdemo.asc hx8kdemo.bin

hx8kprog: hx8kdemo.bin firmware.bin
	ssh pi@zero-one 'icoprog -f' < hx8kdemo.bin
	ssh pi@zero-one 'icoprog -O16 -f' < firmware.bin

prog: 
	ssh pi@zero-one 'icezprog -' < hx8kdemo.bin
	#ssh pi@zero-one 'icoprog -O16 -f' < firmware.bin
...
firmware.elf: sections.lds start.s firmware.c
	$(RISCV_TOOLS_PREFIX)riscv32-unknown-elf-gcc -march=rv32imc -Wl,-Bstatic,-T,sections.lds,--strip-debug -ffreestanding -nostdlib -o firmware.elf start.s firmware.c

firmware.hex: firmware.elf
	$(RISCV_TOOLS_PREFIX)riscv32-unknown-elf-objcopy -O verilog firmware.elf /dev/stdout | sed -e '1 s/@00000000/@00100000/; 2,65537 d;' > firmware.hex

firmware.bin: firmware.elf
	$(RISCV_TOOLS_PREFIX)riscv32-unknown-elf-objcopy -O binary firmware.elf /dev/stdout | tail -c +1048577 > firmware.bin
